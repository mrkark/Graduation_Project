...
# Back patch: strictly above the belt (Y >= 58.0)
t_bk = [t for t in tris if t['center'][2] < -1.5 and 58.0 <= t['center'][1] <= 76.0 and abs(t['center'][0]) < 12.0 and not any('Hand' in b or 'Arm' in b for b in t['bones'])]
...
for name, tlist in [('Left Chest', t_lc), ('Right Arm', t_ra), ('Back', t_bk)]:
    ...
    print(f'1. {name}: {np.sum(dilated)} patch pixels replaced with clean white kimono fabric')

# 4. EXTEND SLEEVES TO WRIST (Arawaza competition cut)
t_ext = [t for t in tris if any('Arm' in b or 'Hand' in b for b in t['bones']) and 27.5 <= abs(t['center'][0]) < 37.0]
m_ext = get_mask(t_ext)

# Cuff opening shadow / fold line (36.0 <= |X| < 37.0)
t_cuff = [t for t in tris if any('Arm' in b or 'Hand' in b for b in t['bones']) and 36.0 <= abs(t['center'][0]) < 37.0]
m_cuff = get_mask(t_cuff)

# Fill sleeve extension with matching kimono fabric
result[m_ext > 0] = synthetic_cloth[m_ext > 0]
# Add natural cuff fold/inner hem shadow so it looks like an open, wide kimono sleeve
cuff_shadow = np.clip(result[m_cuff > 0].astype(int) - 28, 100, 160).astype(np.uint8)
result[m_cuff > 0] = cuff_shadow
print(f'2. Sleeves extended to wrist (|X| < 37.0, Arawaza cut): {np.sum(m_ext > 0)} px')

# 5. PURE BLACK BELT (Zero white artifacts front and back)
# Belt covers 48 <= Y <= 58 on torso, plus knot & dangling ends
belt_knot = [t for t in tris if t['center'][2] > 5.0 and 32.0 <= t['center'][1] <= 55.0 and abs(t['center'][0]) < 7.0]
belt_waist = [t for t in tris if 48.0 <= t['center'][1] <= 58.0 and abs(t['center'][0]) < 12.0 and not any('Arm' in b or 'Hand' in b for b in t['bones'])]
belt_tris = belt_knot + belt_waist
m_belt = get_mask(belt_tris) & (~m_skin_chest)

dark_belt = (m_belt > 0) & (g_orig < 125)
closed_belt = cv2.morphologyEx(dark_belt.astype(np.uint8), cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_RECT, (13, 13))) > 0
...
print(f'4. Chest skin saturation: {c_sat.mean():.1f} (natural bare skin preserved)')
...
for name, tlist in [('Left Chest', t_lc), ('Right Arm', t_ra), ('Back', t_bk)]:
    ...
    assert dark_rem == 0 and sat_rem == 0, f'Failed {name}: dark={dark_rem}, sat={sat_rem}'
    print(f'   -> Verified {name}: 0 dark pixels, 0 colored pixels')

# Belt verification (both front and back):
...
print('\nTexture generated and verified successfully!')
